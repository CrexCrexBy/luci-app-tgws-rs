...
- **Worker-домен проверяется отдельно**, и флаг «Use Cloudflare worker»
  включается/выключается автоматически по его доступности
  (если «Watchdog manages worker» включён).
...
отдельный лог ватчдога (вкладка «Логи»), а интервал и лимиты настраиваются
...
4. Скопируйте адрес вида `<имя>.workers.dev` и впишите его в поле
   **«Cloudflare worker domain»** во вкладке «Настройки».
